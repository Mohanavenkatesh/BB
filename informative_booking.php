...
$cus_num=$_POST['usernum'];
$cus_email=$_POST['uemail'];
$cus_event_name=$_POST['eventName'];
$cus_note=$_POST['note'];
...
if( trim($cus_name)!="user name" && trim($cus_date)!="event date" && trim($cus_num)!="your number" && trim($cus_email)!="your email" && trim($cus_event)!="your event" && trim($cus_area)!="your area" && trim($cus_ppl)!="No of people"  && trim($cus_event_name)!="event name"  && trim($cus_note)!="Note")
{
	if(filter_var($cus_email, FILTER_VALIDATE_EMAIL))
	{
		...
		$message_user='
			...
			<p style="font-size: 16px; margin-bottom:20px;">  Name: '.$cus_name.'</p>
			<p style="font-size: 16px; margin-bottom:20px;">  Email: '.$cus_email.'</p>
			<p style="font-size: 16px; margin-bottom:20px;">  Phone No: '.$cus_num.'</p>
		    <p style="font-size: 16px; margin-bottom:20px;">  Event Name: '.$cus_event_name.'</p>
			<p style="font-size: 16px; margin-bottom:20px;">  Event Date: '.$cus_date.'</p>
		    <p style="font-size: 16px; margin-bottom:20px;">  Event Message: '.$cus_note.'</p>
